...
// @updateURL     http://seanloos.com/userscripts/slImgurAlbums.user.js
// @author        Sean Loos
...
function init() {
	...
	function onKeyDown(e) {
		if (e.altKey || /INPUT|SELECT|TEXTAREA|CANVAS/i.test(e.target.tagName))
			return;
		var ctrl = e.ctrlKey,
		key = e.keyCode,
		shift = e.shiftKey;
	...
	function go(where) {
		...
		console.log(item);
		if (item) {
			img.src = item.attributes['data-href'].value;
		}
		if (i == imgs.length) {
			img.src = '';
